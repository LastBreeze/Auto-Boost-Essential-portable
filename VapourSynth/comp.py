...
def run_comparison():
    ...
    if oxipng_path.exists():
        ...
        if png_files:
            ...
            with Progress(TextColumn("{task.description}"), BarColumn(), TextColumn("{task.percentage:>3.0f}%"), TimeRemainingColumn()) as progress:
                task = progress.add_task(f"Applying lossless oxipng optimization ({oxipng_workers} Threads)", total=len(png_files))
